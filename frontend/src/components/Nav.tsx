...
export default function Nav() {
  return (
    <>
      <nav className="sticky top-0 z-50 flex flex-row justify-between items-center px-6 h-16 lg:h-[6rem] backdrop-blur- bg-black/40 border-b border-white/10">
        <div className="flex flex-row gap-2 text-white items-center">
          <img src={Logo} alt="logo" className="h-5 lg:h-8" />
          <h1 className="text-xl lg:text-3xl font-semibold">BookStack</h1>
        </div>
        <div className="absolute left-1/2 -translate-x-1/2 flex flex-row gap-10 text-white text-2xl font-medium">
          <h2 className="cursor-pointer lg:text-3xl hover:text-purple-400 transition-colors hover:underline decoration-purple-400 decoration-2 underline-offset-8">
            SEARCH
          </h2>
          <h2 className="cursor-pointer lg:text-3xl hover:text-purple-400 transition-colors  hover:underline decoration-purple-400 decoration-2 underline-offset-8">
            LIBRARY
